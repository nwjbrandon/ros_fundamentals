...
PathPlan::PathPlan(ros::NodeHandle& nh)
{
  ...
  explored.insert(std::pair<int, int>(0, 0));
...
void PathPlan::setNextDestCell()
{
  ...
  for(int direction = NORTH; direction <= WEST; direction++){
    if(!hasWall(pos_x_int, pos_y_int, direction)){
      ...
      if(neighbor_value_(direction) <= min_path_value){
        ...
        const bool is_in = explored.find(std::pair<int,int>(x, y)) != explored.end();
        if (is_in) {
          continue;
        }
  ...
  ROS_INFO("Size of history: %d Size of explore %d", history_size, explored_size);
